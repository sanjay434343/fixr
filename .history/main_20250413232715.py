...
def chat_loop(model, tokenizer, device):
    ...
    while True:
        ...
        # Craft a better prompt for Q&A
        prompt = (
            f"{system_prompt}"
            f"Question: {user_input}\n"
            f"Answer: Let me provide a clear answer to your question."
        )
        ...
        with torch.no_grad():
            output = model.generate(
                input_ids,
                max_length=len(input_ids[0]) + 150,  # Increased max length for better answers
                temperature=0.7,
